...
// Endpoint para ver pagos huérfanos (sin suscripción)
logsRouter.get("/payments/orphaned", async (req, res) => {
  const limit = Math.min(500, Math.max(1, Number(req.query.limit ?? 50)));
  const daysOld = Math.max(1, Number(req.query.days ?? 30));
    ...
    return {
      ...
      _analysis: {
        ...
        customerAge: customer.createdAt ? 
          Math.floor((Date.now() - new Date(customer.createdAt).getTime()) / 86400000) : null
      }
    };
  });
