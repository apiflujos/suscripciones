...
exports.normalizeSmartViewScope = normalizeSmartViewScope;
exports.normalizeSmartViewVisibility = normalizeSmartViewVisibility;
exports.normalizeSmartViewType = normalizeSmartViewType;
exports.parseFiltersParam = parseFiltersParam;
exports.listSmartViews = listSmartViews;
exports.getSmartViewById = getSmartViewById;
exports.getSmartViewItemIds = getSmartViewItemIds;
exports.setSmartViewItems = setSmartViewItems;
exports.resolveSmartViewIds = resolveSmartViewIds;
exports.getSmartViewFields = getSmartViewFields;
exports.getSmartViewOptions = getSmartViewOptions;
exports.computeSmartViewIds = computeSmartViewIds;
...
function normalizeFieldOps(fields) {
    return fields.map((f) => ({ ...f, operators: OPS_BY_TYPE[f.type] || f.operators }));
}
function normalizeSmartViewScope(value) {
...
function normalizeSmartViewVisibility(value) {
    const v = String(value || "").trim().toUpperCase();
    return v === "PRIVATE" ? "PRIVATE" : "ORG";
}
function normalizeSmartViewType(value) {
    const v = String(value || "").trim().toUpperCase();
...
async function listSmartViews(scope, tenantId, actorEmail) {
    const visibilityWhere = actorEmail
        ? { OR: [{ visibility: client_1.SmartViewVisibility.ORG }, { visibility: client_1.SmartViewVisibility.PRIVATE, createdByEmail: actorEmail }] }
        : { visibility: client_1.SmartViewVisibility.ORG };
    const items = await prisma_1.prisma.smartView.findMany({
        where: {
            tenantId,
            scope,
            ...visibilityWhere
...
async function getSmartViewById(id, tenantId, actorEmail) {
    const view = await prisma_1.prisma.smartView.findFirst({
        where: { id, tenantId }
    });
    if (!view)
        return null;
    if (view.visibility === "PRIVATE" && actorEmail && view.createdByEmail && view.createdByEmail !== actorEmail)
        return null;
    if (view.visibility === "PRIVATE" && !actorEmail)
        return null;
    return view;
}
async function getSmartViewItemIds(viewId) {
    const items = await prisma_1.prisma.smartViewItem.findMany({
        where: { smartViewId: viewId },
        select: { itemId: true }
    });
    return items.map((i) => String(i.itemId));
}
async function setSmartViewItems(viewId, ids) {
    const unique = Array.from(new Set(ids.filter(Boolean)));
    await prisma_1.prisma.$transaction([
        prisma_1.prisma.smartViewItem.deleteMany({ where: { smartViewId: viewId } }),
        prisma_1.prisma.smartViewItem.createMany({ data: unique.map((itemId) => ({ smartViewId: viewId, itemId })), skipDuplicates: true }),
        prisma_1.prisma.smartView.update({ where: { id: viewId }, data: { staticCount: unique.length } })
    ]);
    return unique.length;
}
async function resolveSmartViewIds(scope, tenantId, actorEmail, viewId, rules) {
    if (viewId) {
        if (!tenantId)
            return [];
        ...
        if (!view || view.scope !== scope)
            return [];
        if (view.type === "STATIC")
            return getSmartViewItemIds(view.id);
        const parsedRules = view.filters;
        if (!parsedRules)
            return [];
        return computeSmartViewIds(scope, tenantId, parsedRules);
    }
    if (rules)
        return computeSmartViewIds(scope, tenantId, rules);
    return null;
}
function getSmartViewFields(scope) {
    if (scope === "customers") {
...
async function getSmartViewOptions(scope, field, tenantId) {
    if (field === "address.city") {
        ...
        rows.forEach((r) => {
            ...
            const city = address.city || meta.city;
            if (city)
                set.add(String(city));
        });
    ...
    if (field === "address.dept") {
        ...
        rows.forEach((r) => {
            const meta = asRecord(r.metadata);
            const address = asRecord(meta.address);
            const dept = address.dept || meta.dept;
            if (dept)
                set.add(String(dept));
        });
        return Array.from(set).sort().map((v) => ({ value: v, label: v }));
    ...
    if (field === "plan.name") {
        const rows = await prisma_1.prisma.subscriptionPlan.findMany({
            where: tenantId ? { OR: [{ tenantId }, { tenantLinks: { some: { tenantId } } }] } : {},
            select: { name: true }
        });
...
async function computeSmartViewIds(scope, tenantId, rules) {
    if (scope === "customers") {
        ...
        return customers
            ...
            const collectionMode = planMeta.collectionMode || null;
            ...
            const ctx = {
                ...
                address: {
                    city: addressMeta.city || customerMeta.city || null,
                    dept: addressMeta.dept || customerMeta.dept || null,
                    line1: addressMeta.line1 || null,
                    postalCode: addressMeta.code5 || addressMeta.postalCode || customerMeta.postalCode || null
                },
                id: {
                    type: customerMeta.identificacionTipo || null,
                    number: customerMeta.identificacionNumero || null
                },
                subscription: {
                    status: sub?.status ?? null,
                    hasActive: sub?.status === client_1.SubscriptionStatus.ACTIVE,
    ...
    if (scope === "products") {
        const items = await prisma_1.prisma.subscriptionPlan.findMany({
            where: {
                metadata: { path: ["kind"], equals: "CATALOG_ITEM" },
                ...(tenantId ? { OR: [{ tenantId }, { tenantLinks: { some: { tenantId } } }] } : {})
            }
    ...
    if (scope === "logs") {
        const items = await prisma_1.prisma.systemLog.findMany({ orderBy: { createdAt: "desc" }, take: 2000 });
        return items
            .filter((l) => {
            const ctx = {
                log: {
    ...
    const items = await prisma_1.prisma.campaign.findMany({
        where: tenantId ? { tenantId } : {},
        orderBy: { createdAt: "desc" }
    });
    return items
