...
exports.sendChatwootMessage = sendChatwootMessage;
...
async function downloadAttachment(url) {
    ...
    if (!contentType.startsWith("image/"))
        throw new Error("attachment_not_image");
...
function stripAttachmentLine(content, url) {
...
function sanitizeInlineImages(content) {
    const safe = String(content || "");
    const lines = safe.split(/\r?\n/);
    const filtered = lines.filter((line) => {
        const trimmed = line.trim();
...
function toContactableInboxes(raw) {
    const payload = raw && typeof raw === "object" && Array.isArray(raw.payload)
        ? raw.payload
        : Array.isArray(raw)
            ? raw
            : [];
    return payload
        ...
        .filter((item) => item.inboxId || item.sourceId);
}
function isWhatsappChannel(meta) {
...
async function sendChatwootMessage(chatwootMessageId) {
    const msg = await prisma_1.prisma.chatwootMessage.findUnique({
        where: { id: chatwootMessageId },
        include: { customer: true, subscription: true }
    });
    if (!msg)
        return;
    if (msg.status === client_1.MessageStatus.SENT)
        return;
    if (!msg.customer?.phone) {
        const errorMessage = "customer_phone_required";
        await prisma_1.prisma.chatwootMessage.update({
            where: { id: chatwootMessageId },
            data: { status: client_1.MessageStatus.FAILED, errorMessage }
        }).catch((err) => {
            logger_1.logger.warn({ err, chatwootMessageId }, "chatwoot.send: failed to update message status");
        });
        await (0, systemLog_1.systemLog)(client_1.LogLevel.WARN, "chatwoot.send", "Cliente sin teléfono", {
            actor: "job:sendChatwootMessage",
            chatwootMessageId,
            customerId: msg.customerId
        }).catch((err) => {
            logger_1.logger.warn({ err, chatwootMessageId }, "chatwoot.send: failed to write system log");
        });
        return;
    }
    ...
    if (!cfg.configured) {
        await prisma_1.prisma.chatwootMessage.update({
            where: { id: chatwootMessageId },
            data: { status: client_1.MessageStatus.FAILED, errorMessage: "chatwoot not configured" }
        });
        await (0, systemLog_1.systemLog)(client_1.LogLevel.WARN, "chatwoot.send", "Chatwoot no configurado", {
            actor: "job:sendChatwootMessage",
            chatwootMessageId,
            customerId: msg.customerId
        }).catch((err) => {
    ...
    const sourceIdDelTelefono = (0, chatwootSync_1.normalizeWhatsappSourceId)(msg.customer.phone);
    // Ensure contact + conversation
    let contactId;
    let sourceId;
    let selectedInboxId;
    let selectedChannel = null;
    let contactableInboxes = [];
    const customerMeta = (msg.customer.metadata ?? {});
    ...
    if (typeof knownContactId === "number" && Number.isFinite(knownContactId)) {
        ...
        if ((0, chatwootSync_1.isValidWhatsappSourceId)(knownSourceId))
            sourceId = knownSourceId.trim();
    }
    ...
    if (contactId && !sourceId) {
        const repaired = await (0, chatwootSync_1.ensureChatwootContactForCustomer)(msg.customerId);
        if (repaired.ok && (0, chatwootSync_1.isValidWhatsappSourceId)(repaired.sourceId)) {
            contactId = repaired.contactId;
            sourceId = repaired.sourceId;
        }
    }
    if (!contactId) {
        const ensured = await (0, chatwootSync_1.ensureChatwootContactForCustomer)(msg.customerId);
        if (ensured.ok) {
            contactId = ensured.contactId;
            sourceId = ensured.sourceId;
        }
        else {
            const errorMessage = ensured.reason === "customer_phone_required"
            ...
            await (0, systemLog_1.systemLog)(client_1.LogLevel.WARN, "chatwoot.send", "No se pudo crear o encontrar el contacto en Chatwoot", {
                actor: "job:sendChatwootMessage",
                chatwootMessageId,
                customerId: msg.customerId,
                reason: ensured.reason,
                detail: "detail" in ensured ? ensured.detail : null,
                searchQueries: "searchQueries" in ensured ? ensured.searchQueries : [],
    ...
    if (!contactId) {
        await prisma_1.prisma.chatwootMessage.update({
            where: { id: chatwootMessageId },
            data: { status: client_1.MessageStatus.FAILED, errorMessage: "contact not found/created" }
        });
        await (0, systemLog_1.systemLog)(client_1.LogLevel.WARN, "chatwoot.send", "Contacto no encontrado/creado", {
            actor: "job:sendChatwootMessage",
            chatwootMessageId,
            customerId: msg.customerId
        }).catch((err) => {
    ...
    const providerResp = (msg.providerResp ?? {});
    ...
    try {
        ...
        logger_1.logger.warn({ err, contactId }, "chatwoot.send: failed to list contactable inboxes");
        contactableInboxes = [];
    }
    if (contactableInboxes.length) {
        ...
        if (!sourceId && prefer?.sourceId)
            sourceId = prefer.sourceId;
    }
    ...
    if (wantsTemplate && !selectedInboxId) {
        ...
        await prisma_1.prisma.chatwootMessage.update({
            ...
            logger_1.logger.warn({ err: updateErr, chatwootMessageId }, "chatwoot.send: failed to update message status");
        });
        await (0, systemLog_1.systemLog)(client_1.LogLevel.WARN, "chatwoot.send", "Plantilla WhatsApp omitida: inbox WhatsApp no disponible", {
            actor: "job:sendChatwootMessage",
            chatwootMessageId,
            customerId: msg.customerId,
    ...
    if (!sourceId) {
        try {
            ...
            await prisma_1.prisma.customer.update({
                ...
                logger_1.logger.warn({ err, customerId: msg.customerId }, "chatwoot.send: failed to update customer metadata");
            });
        }
        catch {
            // ignore
    ...
    await (0, chatwootSync_1.syncChatwootAttributesForCustomer)(msg.customerId).catch((err) => {
        logger_1.logger.warn({ err, customerId: msg.customerId }, "chatwoot.send: failed to sync chatwoot attributes");
    });
    const meta = (msg.subscription?.metadata ?? {});
    ...
    if (typeof existingConversationId === "number") {
        ...
        try {
            conversationId = (await client.createConversation({ contactId, sourceId, inboxId: selectedInboxId ?? cfg.inboxId, message: undefined })).conversationId;
        }
        catch (err) {
            ...
            if (!conversationId && isNotFound) {
                try {
                    const cleanedMeta = customerMeta && typeof customerMeta === "object" ? { ...customerMeta, chatwoot: undefined } : customerMeta;
                    if (cleanedMeta && typeof cleanedMeta === "object") {
                        await prisma_1.prisma.customer
                            .update({
                            where: { id: msg.customerId },
                            data: { metadata: cleanedMeta }
                        })
                            .catch((updateErr) => {
                            logger_1.logger.warn({ err: updateErr, customerId: msg.customerId }, "chatwoot.send: failed to clear customer metadata");
                        });
                    }
                    ...
                    await prisma_1.prisma.customer.update({
                        ...
                        logger_1.logger.warn({ err: updateErr, customerId: msg.customerId }, "chatwoot.send: failed to update customer metadata");
                    });
                    conversationId = (await client.createConversation({ contactId, sourceId, inboxId: selectedInboxId ?? cfg.inboxId, message: undefined })).conversationId;
                }
                catch (recreateErr) {
                    ...
                    await prisma_1.prisma.chatwootMessage.update({
                        where: { id: chatwootMessageId },
                        data: { status: client_1.MessageStatus.FAILED, errorMessage: retryMessage }
                    }).catch((updateErr) => {
                        logger_1.logger.warn({ err: updateErr, chatwootMessageId }, "chatwoot.send: failed to update message status");
                    });
                    await (0, systemLog_1.systemLog)(client_1.LogLevel.ERROR, "chatwoot.send", "Error creando conversación", {
                        actor: "job:sendChatwootMessage",
                        chatwootMessageId,
                        customerId: msg.customerId,
                        err: retryMessage
                    }).catch((logErr) => {
                        logger_1.logger.warn({ err: logErr, chatwootMessageId }, "chatwoot.send: failed to write system log");
                    });
                    return;
                }
            }
                ...
                await prisma_1.prisma.chatwootMessage.update({
                    where: { id: chatwootMessageId },
                    data: { status: client_1.MessageStatus.FAILED, errorMessage: message }
                }).catch((updateErr) => {
                    logger_1.logger.warn({ err: updateErr, chatwootMessageId }, "chatwoot.send: failed to update message status");
                });
                await (0, systemLog_1.systemLog)(client_1.LogLevel.ERROR, "chatwoot.send", "Error creando conversación", {
    ...
    if (typeof conversationId !== "number") {
        await prisma_1.prisma.chatwootMessage.update({
            where: { id: chatwootMessageId },
            data: { status: client_1.MessageStatus.FAILED, errorMessage: "chatwoot_conversation_missing" }
        }).catch((updateErr) => {
            logger_1.logger.warn({ err: updateErr, chatwootMessageId }, "chatwoot.send: failed to update message status");
        });
        await (0, systemLog_1.systemLog)(client_1.LogLevel.ERROR, "chatwoot.send", "Conversación no disponible", {
            actor: "job:sendChatwootMessage",
            chatwootMessageId,
            customerId: msg.customerId
        }).catch((logErr) => {
    ...
    if (typeof existingConversationId !== "number" && msg.subscriptionId) {
        ...
        await prisma_1.prisma.subscription.update({
            ...
            logger_1.logger.warn({ err, subscriptionId: msg.subscriptionId }, "chatwoot.send: failed to update subscription metadata");
        });
    }
    ...
    if (templateParams) {
        try {
            ...
            if (!isWhatsapp) {
                await prisma_1.prisma.chatwootMessage.update({
                    where: { id: chatwootMessageId },
                    data: { status: client_1.MessageStatus.FAILED, errorMessage: "whatsapp_channel_required" }
                }).catch((updateErr) => {
                    logger_1.logger.warn({ err: updateErr, chatwootMessageId }, "chatwoot.send: failed to update message status");
                });
                await (0, systemLog_1.systemLog)(client_1.LogLevel.WARN, "chatwoot.send", "Template omitido: canal no WhatsApp", {
                    actor: "job:sendChatwootMessage",
                    chatwootMessageId,
                    customerId: msg.customerId,
                    channelType,
            ...
            await prisma_1.prisma.chatwootMessage.update({
                where: { id: chatwootMessageId },
                data: { status: client_1.MessageStatus.FAILED, errorMessage: "whatsapp_channel_lookup_failed" }
            }).catch((updateErr) => {
                logger_1.logger.warn({ err: updateErr, chatwootMessageId }, "chatwoot.send: failed to update message status");
            });
            await (0, systemLog_1.systemLog)(client_1.LogLevel.WARN, "chatwoot.send", "No se pudo validar canal WhatsApp para template", {
                actor: "job:sendChatwootMessage",
                chatwootMessageId,
                customerId: msg.customerId
            }).catch((logErr) => {
    ...
    const doSend = async (convId) => {
        ...
        if (attachmentUrl) {
            try {
                ...
                await (0, systemLog_1.systemLog)(client_1.LogLevel.WARN, "chatwoot.send", "Adjunto falló; enviando solo texto", {
                    actor: "job:sendChatwootMessage",
                    chatwootMessageId,
                    customerId: msg.customerId,
    ...
    try {
        ...
        const isStaleConversation = typeof existingConversationId === "number" &&
            conversationId === existingConversationId &&
        ...
        if (isStaleConversation && msg.subscriptionId) {
            ...
            delete cleanedSubMeta.chatwoot.conversationId;
            await prisma_1.prisma.subscription.update({
                where: { id: msg.subscriptionId },
                data: { metadata: cleanedSubMeta }
            }).catch((updateErr) => {
                logger_1.logger.warn({ err: updateErr, subscriptionId: msg.subscriptionId }, "chatwoot.send: failed to clear stale conversationId");
            });
            try {
                ...
                await prisma_1.prisma.subscription.update({
                    where: { id: msg.subscriptionId },
                    data: { metadata: refreshedSubMeta }
                }).catch((updateErr) => {
                    logger_1.logger.warn({ err: updateErr, subscriptionId: msg.subscriptionId }, "chatwoot.send: failed to save new conversationId after recovery");
                });
                await (0, systemLog_1.systemLog)(client_1.LogLevel.WARN, "chatwoot.send", "Conversacion obsoleta recreada y mensaje enviado", {
                    actor: "job:sendChatwootMessage",
                    chatwootMessageId,
                    customerId: msg.customerId,
                ...
                await (0, systemLog_1.systemLog)(client_1.LogLevel.ERROR, "chatwoot.send", "Error enviando mensaje tras recrear conversacion", {
                    actor: "job:sendChatwootMessage",
                    chatwootMessageId,
                    customerId: msg.customerId,
                    err: retryMessage
            ...
            await (0, systemLog_1.systemLog)(client_1.LogLevel.ERROR, "chatwoot.send", "Error enviando mensaje", {
                actor: "job:sendChatwootMessage",
                chatwootMessageId,
                customerId: msg.customerId,
                err: message
    ...
    await prisma_1.prisma.chatwootMessage.update({
        ...
        data: {
            ...
            providerResp: templateParams
                ? { ...msg.providerResp, response: sent.raw }
                : sent.raw,
            errorMessage: null
        }
    });
    ...
    const delivery = await (0, chatwootDelivery_1.reconcileChatwootMessageDelivery)({
        chatwootMessageId,
        actor: "job:sendChatwootMessage",
        attempts: 4,
        waitMs: 1500
    ...
    if (!delivery.ok) {
        await prisma_1.prisma.chatwootMessage.update({
            ...
            logger_1.logger.warn({ err, chatwootMessageId }, "chatwoot.send: failed to persist fallback pending state");
        });
        await (0, systemLog_1.systemLog)(client_1.LogLevel.WARN, "chatwoot.send", "No se pudo reconciliar entrega real en Chatwoot", {
            actor: "job:sendChatwootMessage",
            chatwootMessageId,
            customerId: msg.customerId,
            reason: delivery.reason
        }).catch((err) => {
            logger_1.logger.warn({ err, chatwootMessageId }, "chatwoot.send: failed to write delivery reconciliation log");
        });
    }
    if (delivery.ok ? delivery.state === "delivered" || delivery.state === "submitted" : false) {
        await (0, superAdminApp_1.consumeApp)("messages_sent", { amount: 1, source: "jobs:chatwoot.sent", meta: { type: msg.type, id: msg.id } });
        if (msg.type === client_1.ChatwootMessageType.PAYMENT_LINK) {
            await (0, superAdminApp_1.consumeApp)("payment_links_sent", { amount: 1, source: "jobs:chatwoot.sent", meta: { chatwootMessageId: msg.id } });
        }
    }
}
